...
ImVec2 ImGui::CalcTextSizeW( const wchar_t* text, const wchar_t* text_end, bool hide_text_after_double_hash, float wrap_width )
{
	ImGuiContext& g = *GImGui;

	const wchar_t* text_display_end;
	if( hide_text_after_double_hash )
		text_display_end = ( const wchar_t* )FindRenderedTextEnd( ( const char* )text, ( const char* )text_end );      // Hide anything after a '##' string
	else
		text_display_end = text_end;
	...
	ImVec2 text_size = font->CalcTextSizeW( font_size, FLT_MAX, wrap_width, text, text_display_end, NULL );
...
bool ImGui::Button( const std::string& prefix, const std::string& label, const ImVec2& size /*= { 0.f, 0.f }*/ )
{
	...
	auto prefix_size = CalcTextSize( prefix.c_str(), nullptr, true );
	auto label_size = CalcTextSize( label.c_str(), nullptr, true );

	auto total_size = prefix_size + label_size;

	auto pos = window->DC.CursorPos;
	auto item_size = CalcItemSize( size, total_size.x + style.FramePadding.x * 2.f, total_size.y + style.FramePadding.y * 2.f );
	...
	auto pressed = ButtonBehavior( bb, id, &hovered, &held, 0 );

	const auto color = GetColorU32( ( hovered && held ) ? ImGuiCol_ButtonActive : hovered ? ImGuiCol_ButtonHovered : ImGuiCol_Button );
	RenderFrame( bb.Min, bb.Max, color, true, style.FrameRounding );

	const auto prefix_bb = ImRect( pos + ImVec2( 12.f, 2.f ), pos + ImVec2( size.y, size.y ) );
	const auto label_bb = ImRect( pos + ImVec2( size.y * 1.5f, 0.f ), pos + size );

	RenderTextClipped( prefix_bb.Min, prefix_bb.Max, prefix.c_str(), nullptr, &prefix_size, style.ButtonTextAlign, &prefix_bb );
	RenderTextClipped( label_bb.Min, label_bb.Max, label.c_str(), nullptr, &label_size, style.ButtonTextAlign, &label_bb );
...
bool ImGui::KeyBox( const std::string& label, int* code, const ImVec2& size /*= { 0.f, 0.f }*/ )
{
	...
	{
		auto name = GetNameFromCode( *code );
		text = name;
...
void ImGui::LoadSettings( const char* name )
{
	...
	std::string ini_filename( name );

	if( ini_filename.find( ".theme" ) == std::string::npos )
		ini_filename.append( ".theme" );

	auto file_size = 0;
	auto file_data = reinterpret_cast< char* >( ImFileLoadToMemory( ini_filename.c_str(), "rb", &file_size, 1 ) );
	...
	const auto buf_end = file_data + file_size;

	for( auto line_start = file_data; line_start < buf_end; )
	{
		...
		else if( settings )
		{
			...
			if( sscanf( line_start, "pos=%f,%f", &x, &y ) == 2 )
				...
				settings->Size = ImMax( ImVec2( x, y ), gui.Style.WindowMinSize );
			else if( sscanf( line_start, "collapsed=%d", &i ) == 1 )
				settings->Collapsed = ( i != 0 );
		}
...
void ImGui::SaveSettings( const char* name )
{
	...
	for( auto i = 0; i != gui.Windows.Size; i++ )
	{
		auto window = gui.Windows[ i ];

		if( window->Flags & ImGuiWindowFlags_NoSavedSettings )
			continue;

		auto settings = FindWindowSettings( window->Name );
	...
	auto f = ImFileOpen( ini_filename.c_str(), "wt" );

	if( !f )
		return;

	for( auto i = 0; i != gui.Settings.Size; i++ )
	{
		const auto settings = &gui.Settings[ i ];
		...
		if( const auto p = std::strstr( name, "###" ) )
			name = p;
