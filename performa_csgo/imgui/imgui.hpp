...
// Settings
IMGUI_API void			LoadSettings( const char* name );
IMGUI_API void			SaveSettings( const char* name );
...
IMGUI_API void          Bullet();                                                                     // draw a small circle and keep the cursor on the same line. advance cursor x position by GetTreeNodeToLabelSpacing(), same distance that TreeNode() uses
																									  ...
																									  // Widgets: Main
IMGUI_API bool			Button( const std::string& prefix, const std::string& label, const ImVec2& size = { 0.f, 0.f } );
...
IMGUI_API bool			KeyBox( const std::string& label, int* code, const ImVec2& size = { 0.f, 0.f } );
...
IMGUI_API ImVec2        CalcTextSizeW( const wchar_t* text, const wchar_t* text_end = NULL, bool hide_text_after_double_hash = false, float wrap_width = -1.0f );
...
namespace ImGui
{
static inline bool      IsRootWindowOrAnyChildHovered( ImGuiHoveredFlags flags = 0 )
{
	return IsItemHovered( flags | ImGuiHoveredFlags_FlattenChilds );
...
public:
	...
	{
		int new_capacity = Capacity ? ( Capacity + Capacity / 2 ) : 8; return new_capacity > size ? new_capacity : size;
	}
	...
	{
		if( new_size > Capacity ) reserve( _grow_capacity( new_size ) ); if( new_size > Size ) for( int n = Size; n < new_size; n++ ) Data[ n ] = v; Size = new_size;
	}
...
struct ImDrawList
{
	...
	IMGUI_API void  AddTextW( const ImVec2& pos, ImU32 col, const wchar_t* text_begin, const wchar_t* text_end = NULL );
	IMGUI_API void  AddTextW( const ImFont* font, float font_size, const ImVec2& pos, ImU32 col, const wchar_t* text_begin, const wchar_t* text_end = NULL, float wrap_width = 0.0f, const ImVec4* cpu_fine_clip_rect = NULL );
...
struct ImFontAtlas
{
	...
	struct GlyphRangesBuilder
	{
		...
		GlyphRangesBuilder()
		{
			UsedChars.resize( 0x10000 / 8 ); memset( UsedChars.Data, 0, 0x10000 / 8 );
		}
		bool           GetBit( int n )
		{
...
struct ImFont
{
	...
	IMGUI_API ImVec2            CalcTextSizeW( float size, float max_width, float wrap_width, const wchar_t* text_begin, const wchar_t* text_end = NULL, const wchar_t** remaining = NULL ) const; // utf8
	IMGUI_API const char*       CalcWordWrapPositionA( float scale, const char* text, const char* text_end, float wrap_width ) const;
	IMGUI_API void              RenderChar( ImDrawList* draw_list, float size, ImVec2 pos, ImU32 col, unsigned short c ) const;
	IMGUI_API void              RenderText( ImDrawList* draw_list, float size, ImVec2 pos, ImU32 col, const ImVec4& clip_rect, const char* text_begin, const char* text_end, float wrap_width = 0.0f, bool cpu_fine_clip = false ) const;
	
	// unicode
	IMGUI_API void              RenderTextW( ImDrawList* draw_list, float size, ImVec2 pos, ImU32 col, const ImVec4& clip_rect, const wchar_t* text_begin, const wchar_t* text_end, float wrap_width = 0.0f, bool cpu_fine_clip = false ) const;
