...
void ImDrawList::AddTextW( const ImFont* font, float font_size, const ImVec2& pos, ImU32 col, const wchar_t* text_begin, const wchar_t* text_end, float wrap_width, const ImVec4* cpu_fine_clip_rect )
{
	if( ( col & IM_COL32_A_MASK ) == 0 )
		return;
	...
	font->RenderTextW( this, font_size, pos, col, clip_rect, text_begin, text_end, wrap_width, cpu_fine_clip_rect != NULL );
}
...
void ImDrawList::AddTextW( const ImVec2& pos, ImU32 col, const wchar_t* text_begin, const wchar_t* text_end )
{
	AddTextW( NULL, 0.0f, pos, col, text_begin, text_end );
}
...
ImVec2 ImFont::CalcTextSizeW( float size, float max_width, float wrap_width, const wchar_t* text_begin, const wchar_t* text_end, const wchar_t** remaining ) const
{
	if( !text_end )
	...
	const char* s = ( const char* )text_begin;
	while( s < ( const char* )text_end )
	{
		if( word_wrap_enabled )
		{
			...
			if( !word_wrap_eol )
			{
				word_wrap_eol = CalcWordWrapPositionA( scale, s, ( const char* )text_end, wrap_width - line_width );
		...
		{
			s += ImTextCharFromUtf8( &c, s, ( const char* )text_end );
...
void ImFont::RenderTextW( ImDrawList* draw_list, float size, ImVec2 pos, ImU32 col, const ImVec4& clip_rect, const wchar_t* text_begin, const wchar_t* text_end, float wrap_width /*= 0.0f*/, bool cpu_fine_clip /*= false*/ ) const
{
	if( !text_end )
		text_end = text_begin + wcslen( text_begin ); // ImGui functions generally already provides a valid text_end, so this is merely to handle direct calls.
	...
	if( !word_wrap_enabled && y + line_height < clip_rect.y )
		while( s < ( const char* )text_end && *s != '\n' )  // Fast-forward to next line
			s++;

	// Reserve vertices for remaining worse case (over-reserving is useful and easily amortized)
	const int vtx_count_max = ( int )( ( const char* )text_end - s ) * 4;
	const int idx_count_max = ( int )( ( const char* )text_end - s ) * 6;
	...
	while( s < ( const char* )text_end )
	{
		if( word_wrap_enabled )
		{
			...
			if( !word_wrap_eol )
			{
				word_wrap_eol = CalcWordWrapPositionA( scale, s, ( const char* )text_end, wrap_width - ( x - pos.x ) );
				if( word_wrap_eol == s ) // Wrap_width is too small to fit anything. Force displaying 1 character to minimize the height discontinuity.
					word_wrap_eol++;    // +1 may not be a character start point in UTF-8 but it's ok because we use s >= word_wrap_eol below
			}
