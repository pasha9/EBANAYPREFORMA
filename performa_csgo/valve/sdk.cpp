...
{
	return ( m_pEnt == source::m_client_entity_list->GetClientEntity( 0 )->GetBaseEntity() );
}
...
{
	m_cvar_restore = source::m_cvar->FindVar( var_name );
	Spoof();
}
...
SpoofedConVar::~SpoofedConVar()
{
	if( IsSpoofed() )
	{
		...
		source::m_cvar->UnregisterConCommand( m_cvar_replace );
		free( m_cvar_replace );
...
void SpoofedConVar::Spoof()
{
	if( !IsSpoofed() && m_cvar_restore )
	{
		...
		source::m_cvar->RegisterConCommand( m_cvar_replace );
...
{
	using Fn = bool( __thiscall* )( void*, const char*, const char*, void*, const char*, unsigned long );
	static auto procedure = memory::scan< Fn >( "client.dll", "55 8B EC 83 E4 F8 83 EC 34 53 8B 5D 0C 89 4C 24 04" );
	return procedure( this, resourceName, pBuffer, pFileSystem, pPathID, 0u );
}

auto IMaterialSystem::m_bGameStarted() -> bool&
{
	static auto offset = memory::scan< std::uintptr_t >( "materialsystem.dll", "80 B9 ? ? ? ? ? 74 0F", 2, 1u );
	return *( bool* )( this + offset );
}

auto IMaterialSystem::ForceBeginRenderTargetAllocation() -> void
{
	auto bGameStarted = m_bGameStarted();
	m_bGameStarted() = false;
	BeginRenderTargetAllocation();
	m_bGameStarted() = bGameStarted;
}

auto IMaterialSystem::ForceEndRenderTargetAllocation() -> void
{
	auto bGameStarted = m_bGameStarted();
	m_bGameStarted() = false;
...
auto IMaterialSystem::CreateFullFrameRenderTarget( const char* name ) -> ITexture*
{
